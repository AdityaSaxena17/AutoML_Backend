...
public class SvmModelStrategy implements ModelStrategy {

    private final EcsClient ecsClient;

    @Override
    public void execute(JobRequestDto request) {
        System.out.println("--- Starting SVM Instantiation for Job: " + request.getJobid() + " ---");
        ...
        try {
            ...
            ContainerOverride containerOverride = ContainerOverride.builder()
                    ...
                    .environment(
                            ...
                            KeyValuePair.builder().name("MODEL_TYPE").value("SVM").build() 
                    )
                    .build();
            ...
            if (response.hasTasks()) {
                ...
                System.out.println("SVM Fargate Task Launched Successfully. ARN: " + taskArn);
            } else {
                System.err.println("Task launch failed: " + response.failures().toString());
            }

        } catch (Exception e) {
            System.err.println("Error in SvmModelStrategy: " + e.getMessage());
            throw new RuntimeException("Failed to spin up SVM container", e);
        }
    }
